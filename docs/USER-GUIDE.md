...
2. Run **Import / Refresh Generated Lineup** on the Actions tab and confirm that the imported lineup will become active and **Match Sensitivity** will be set to **Exact**.
3. Every successful import or refresh saves both settings together, preserving all other settings. You may need to disable and re-enable the plugin for these changes to show correctly in Settings. If the download or validation fails, both settings stay unchanged. You can change sensitivity afterward, but the next successful import or refresh sets it back to Exact.
